...
def ejecutar_instancia(
    ...
    ruta_hexagonos = os.path.join(carpeta, f"grafo_hexagons_{m}m.xlsx")
    ruta_distancias = os.path.join(carpeta, f"distancias_{m}m.xlsx")
    ...
    if (use_integer_mincut and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_tesselation_mincut_strengh_{m}m_alfa{alfa}_n{n}.xlsx"
        )

    elif (use_integer_mincut and not use_strengthening):
            ruta_excel_salida = os.path.join(
                carpeta,
                f"Districting_tesselation_mincut_{m}m_alfa{alfa}_n{n}.xlsx"
            )

    elif (use_integer_minimal and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_tesselation_minimal_strengh_{m}m_alfa{alfa}_n{n}.xlsx"
        )

    elif (use_integer_minimal and not use_strengthening):
            ruta_excel_salida = os.path.join(
                carpeta,
                f"Districting_tesselation_minimal_{m}m_alfa{alfa}_n{n}.xlsx"
            )

    elif (use_fractional_mincut and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_tesselation_frac_mincut_strengh_{m}m_alfa{alfa}_n{n}.xlsx"
        )
...
alfas = [1]
ns = [10]
ms = [1000, 5000, 10000, 15000, 20000, 30000]
