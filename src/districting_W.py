...
def problema_cobertura(hex, alfa):
    ...
    if model_cobertura.SolCount > 0:
        ...
        print(f"\nRestricted set W with radii: {W_radio}")
    else:
...
def ejecutar_instancia(
    ...
    if (use_integer_mincut and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_W_mincut_strengh_{hex}hex_alfa{alfa}_n{n}.xlsx"
        )

    elif (use_integer_mincut and not use_strengthening):
            ruta_excel_salida = os.path.join(
                carpeta,
                f"Districting_W_mincut_{hex}hex_alfa{alfa}_n{n}.xlsx"
            )

    elif (use_integer_minimal and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_W_minimal_strengh_{hex}hex_alfa{alfa}_n{n}.xlsx"
        )

    elif (use_integer_minimal and not use_strengthening):
            ruta_excel_salida = os.path.join(
                carpeta,
                f"Districting_W_minimal_{hex}hex_alfa{alfa}_n{n}.xlsx"
            )

    elif (use_fractional_mincut and use_strengthening):
        ruta_excel_salida = os.path.join(
            carpeta,
            f"Districting_W_frac_mincut_strengh_{hex}hex_alfa{alfa}_n{n}.xlsx"
        )
